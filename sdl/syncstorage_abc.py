...
class SyncLockAbc(ABC):
    """
    An abstract class providing synchronous locking mechanism for shared data layer storage.
    ...
    def acquire(self, retry_interval: Union[int, float] = 0.1,
                retry_timeout: Union[int, float] = 10) -> None:
        """
        ...
        Raises:
            ...
            LockAcquireFailed: If Lock acquiring fails, because someone was keeping the lock.
            ...
            OperationInterrupted: If shareddatalayer does not receive a reply from the backend data
                                  storage.
        """
        pass
...
class SyncStorageAbc(ABC):
    ...
    def set(self, ns: str, data_map: Dict[str, bytes]) -> None:
        ...
        Raises:
            ...
            OperationInterrupted: If shareddatalayer does not receive a reply from the backend data
                                  storage.
            InvalidNamespace: If given namespace does not meet the namespace format restrictions.
        """
        pass
    ...
    @abstractmethod
    def get(self, ns: str, keys: Set[str]) -> Dict[str, bytes]:
        """
        ...
        Args:
            ns (str): Namespace under which this operation is targeted.
            keys (set of str): Keys to be read.
    ...
    def find_and_get(self, ns: str, key_prefix: str, atomic: bool) -> Dict[str, bytes]:
        ...
        NOTE: In atomic action, if used prefix produces huge number of matches, that could have
    ...
    @abstractmethod
    def remove(self, ns: str, keys: Set[str]) -> None:
        """
        ...
        Args:
            ns (str): Namespace under which this operation is targeted.
            keys (set of str): Keys, which data is to be removed.
    ...
    def add_member(self, ns: str, group: str, members: Set[bytes]) -> None:
        """
        Add new members to a shared data layer group.

        Shared data layer groups are unordered collections of members where each member is unique.
        If a member to be added is already a member of the group, its addition is silently ignored.
        If the group does not exist, it is created, and specified members are added to the group.
        All the exceptions except SdlTypeError are derived from SdlException base class. Client
        can catch only that exception if separate handling for different shareddatalayer error
        situations is not needed. Exception SdlTypeError is derived from build-in TypeError and it
        indicates misuse of the SDL API.
        ...
        Args:
            ...
            members (set of bytes): Members to be added.
    ...
    def remove_member(self, ns: str, group: str, members: Set[bytes]) -> None:
        ...
        Args:
            ns (str): Namespace under which this operation is targeted.
            group (str): Group name.
            members (set of bytes): Members to be removed.
