...
class FakeBackend(DbBackendAbc):
    """
    ...
    only its local local 'fake' database. Also keys are stored in database under the same
    namespace.
...
class FakeBackendLock(DbBackendLockAbc):
    """
    ...
    def __init__(self, ns: str, name: str, expiration: Union[int, float],
                 redis_backend: FakeBackend) -> None:
        self._locked = False
        self._ns = ns
