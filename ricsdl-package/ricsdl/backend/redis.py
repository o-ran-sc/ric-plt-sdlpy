...
class RedisBackend(DbBackendAbc):
    ...
    def subscribe_channel(self, ns: str, cb: Callable[[str, str], None],
        ...
        for channel in channels:
            with _map_to_sdl_exception():
                redis_ctx = self.__getClientCtx(ns)
    ...
    def unsubscribe_channel(self, ns: str, channels: List[str]) -> None:
        ...
        for channel in channels:
            with _map_to_sdl_exception():
                self.__getClientCtx(ns).redis_pubsub.unsubscribe(channel)

    def start_event_listener(self) -> None:
        redis_ctxs = self.__getClientCtxs()
    ...
    def __create_redis_client(self, cfg_params, addr):
        ...
        return _RedisClient(new_redis, redis_pubsub, pubsub_thread, run_in_thread)

    def __getClientCtxs(self):
        return self.clients

    def __getClientCtx(self, ns):
