...
from sdl.syncstorage import SyncLock
from sdl.syncstorage import func_arg_checker
from sdl.exceptions import SdlTypeError
...
def sync_storage_fixture(request):
    ...
    with patch('sdl.backend.get_backend_instance') as mock_db_backend:
        storage = SyncStorage()
...
class TestSyncStorage:
    ...
    def test_set_if_function_success(self):
        ...
        self.mock_db_backend.set_if.assert_called_once_with(self.ns, self.key, self.old_data, self.new_data)
        assert ret == True
    ...
    def test_set_if_not_exists_function_success(self):
        ...
        self.mock_db_backend.set_if_not_exists.assert_called_once_with(self.ns, self.key, self.new_data)
        assert ret == True
    ...
    def test_find_and_get_function_success(self):
        ...
        self.mock_db_backend.find_and_get.assert_called_once_with(self.ns, self.keyprefix, self.is_atomic)
        assert ret == self.dm
    ...
    def test_remove_function_success(self):
        ...
        assert call_args[0] == self.ns
        assert type(call_args[1]) is list
        assert len(call_args[1]) == len(self.keys)
        assert all(k in call_args[1] for k in self.keys)
    ...
    def test_remove_all_function_success(self):
        ...
        ret = self.storage.remove_all(self.ns)
        self.mock_db_backend.find_keys.assert_called_once_with(self.ns, '')
        self.mock_db_backend.remove.assert_called_once_with(self.ns, self.mock_db_backend.find_keys.return_value)
    ...
    def test_add_member_function_success(self):
        ...
        self.mock_db_backend.add_member.assert_called_once_with(self.ns, self.group, self.groupmembers)
    ...
    def test_remove_member_function_success(self):
        ...
        self.mock_db_backend.remove_member.assert_called_once_with(self.ns, self.group, self.groupmembers)
    ...
    def test_is_member_function_success(self):
        ...
        self.mock_db_backend.is_member.assert_called_once_with(self.ns, self.group, self.groupmember)
        assert ret == True
    ...
    @patch('sdl.syncstorage.SyncLock')
    ...
    @patch('sdl.syncstorage.SyncLock')
    def test_get_lock_resource_function_success_when_expiration_time_is_float_number(self, mock_db_lock):
...
def lock_fixture(request):
    ...
    with patch('sdl.backend.get_backend_lock_instance') as mock_db_backend_lock:
...
class TestSyncLock:
    def test_acquire_function_success_when_timeout_and_interval_are_integers(self):
        ...
        self.mock_db_backend_lock.acquire.assert_called_once_with(self.retry_interval, self.retry_timeout)
    ...
    def test_acquire_function_success_when_timeout_and_interval_are_float_numbers(self):
        ...
        self.mock_db_backend_lock.acquire.assert_called_once_with(float(self.retry_interval), float(self.retry_timeout))
