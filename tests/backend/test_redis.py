...
import sdl.backend
from sdl.backend.redis import (RedisBackend, RedisBackendLock, _map_to_sdl_exception)
...
class TestRedisBackend:
    ...
    def test_set_if_function_success(self):
        ...
        self.mock_redis.execute_command.assert_called_once_with('SETIE', self.key_redis, self.new_data, self.old_data)
        assert ret == True
    ...
    def test_remove_if_function_success(self):
        ...
        self.mock_redis.execute_command.assert_called_once_with('DELIE', self.key_redis, self.new_data)
        assert ret == True
...
def mock_redis_lock():
    def _mock_redis_lock(redis, name, timeout=None, sleep=0.1,
                         blocking=True, blocking_timeout=None, thread_local=True):
